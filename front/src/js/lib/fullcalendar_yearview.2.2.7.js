            ...
            cellOver: function (cell) {
                ...
                if (!newEnd.isAfter(start)) {
                    ...
                    newEnd = start.clone().add(
                        // make the event span a single slot
...
DayGrid.mixin({
    ...
    fgSegHtml: function (seg, disableResizing) {
        ...
        var isResizable =
            !disableResizing && event.allDay && seg.isEnd && view.isEventResizable(event)
        ...
        if (!event.allDay && seg.isStart) {
            timeHtml =
                '<span class="fc-time">' + htmlEscape(this.getEventTimeText(event)) + "</span>"
        }
...
var TimeGrid = Grid.extend({
    ...
    computeDateTop: function (date, startOfDayDate) {
        return this.computeTimeTop(
            moment.duration(date.clone().stripZone() - startOfDayDate.clone().stripTime()),
        )
    },
...
var View = (fc.View = Class.extend({
    ...
    massageCurrentDate: function (date, direction) {
        if (this.intervalDuration <= moment.duration({ days: 1 })) {
...
function Calendar(element, instanceOptions) {
    ...
    function _calcSize() {
        ...
        if (typeof options.contentHeight === "number") {
            ...
            // exists and not 'auto'
            suggestedViewHeight =
                options.height - (headerElement ? headerElement.outerHeight(true) : 0)
        } else {
...
fcViews.agenda = View.extend({
    ...
    renderHtml: function () {
        return (
            ...
            '">' +
            (this.dayGrid
                ? '<div class="fc-day-grid"/>' + '<hr class="' + this.widgetHeaderClass + '"/>'
                : "") +
    ...
    slotBgIntroHtml: function () {
        return (
            '<td class="fc-axis ' + this.widgetContentClass + '" ' + this.axisStyleAttr() + "></td>"
        )
    },
...
fcViews.year = View.extend({
    ...
    buildSkeleton: function (monthsPerRow, showNumbers) {
        ...
        for (n = 0; n < this.nbMonths; n++) {
            ...
            s += '<table class="fc-year-month-header">' + '<thead><tr class="fc-year-week-days">'
            ...
            s += "</tr><tr>" + "</tr></thead></table>" // fc-year-month-header
...
*/
    ...
    setHeight: function (totalHeight, isAuto) {
        ...
        $.each(this.dayGrids, function (offset, dayGrid) {
            if (dayGrid.el.length > 0) {
                ...
                if (!isAuto && setPotentialScroller(dayGrid.scrollerEl, scrollerHeight)) {
                    // using scrollbars?
    ...
    updateHeight: function () {
        ...
        if (this.yearColumns > 0) {
            var height = calendar.getSuggestedViewHeight() * (1.1 / (0.01 + this.yearColumns))
